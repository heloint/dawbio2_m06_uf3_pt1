...
export class RegisterFormComponent implements OnInit {
  ...
  get registerInformationTypes() {
    return this.localStorageHandler.getLocalStorageValue(
  ...
  registerForm: FormGroup = new FormGroup({
    username: new FormControl(localStorage['registerUserName'], [
      Validators.required,
      Validators.minLength(6),
      Validators.pattern('^[a-zA-Z0-9]+$'),
    ]),

    password: new FormControl(localStorage['registerPassword'], [
      Validators.required,
    ...
    email: new FormControl(localStorage['registerEmail'], [
      Validators.required,
    ...
    gender: new FormControl(localStorage['registerGender'], [
      Validators.required,
    ]),
    ...
    informationTypes: new FormArray(
      this.fetchCheckboxFormArray(
        Array.from(
          Object.values(JSON.parse(localStorage['registerInformationTypes']))
        )
      )
    ),
  ...
  register() {
    const infoInterests: string[] = Array.from(
      Object.values(JSON.parse(localStorage['registerInformationTypes']))
    );
